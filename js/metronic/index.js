var Index = function () {
    ...
    return {
        ...
        initCharts: function (stats_data) {
            if (!jQuery.plot) {
                return;
            }

            function showChartTooltip(x, y, xValue, yValue) {
                $('<div id="tooltip" class="chart-tooltip"><div><strong>Avg. Response Time</strong><\/div><div class="clearfix"><\/div><div class="text-center">' + yValue + '<\/div><\/div>').css({
                    position: 'absolute',
                    display: 'none',
                    top: y - 40,
                    left: x - 40,
                ...
                $("#site_statistics").bind("plothover", function (event, pos, item) {
                    ...
                    if (item) {
                        if (previousPoint != item.dataIndex) {
                            ...
                            showChartTooltip(item.pageX, item.pageY, item.datapoint[0], item.datapoint[1] + ' ms');
                        }
                    } else {
                        $("#tooltip").remove();
                        previousPoint = null;
		...
		initSummaryCharts: function (stats_data, container_id) {
            if (!jQuery.plot) {
                return;
            }
            ...
            var totalPoints = 10;
            var visitors = stats_data;
            if ($('#site_statistics_'+container_id).size() != 0) {

                $('#site_statistics_loading_'+container_id).hide();
                $('#site_statistics_content_'+container_id).show();

                var plot_statistics = $.plot($("#site_statistics_"+container_id),
                    [{
                        data: visitors,
                        lines: {
                            fill: 0.6,
                ...
                var previousPoint = null;
                $("#site_statistics_"+container_id).bind("plothover", function (event, pos, item) {
                    $("#x").text(pos.x.toFixed(2));
                    $("#y").text(pos.y.toFixed(2));
                    if (item) {
                        if (previousPoint != item.dataIndex) {
