	...
	*/
	function UI()
	{
		...
		this.postMappingOperations = function()
		{
			...
			if (config.ui)
			{
				...
				for (var i = 0; i < addresses.length; i++)
				{
					...
					$('#postcode').parent('.form-group').append('<div class="smarty-ui"><div title="Loading..." class="smarty-dots smarty-addr-'+id+'"></div></div>');
					var offset = uiTagOffset(addresses[i].corners(true));
					//top: '+offset.top+'px; left: '+offset.left+'px;
					$('#postcode').parent('.form-group').append('<div class="smarty-ui" style=""><a href="javascript:" class="smarty-tag smarty-tag-grayed smarty-addr-'+id+'" title="Address not verified. Click to verify." data-addressid="'+id+'"><span class="smarty-tag-check">&#10003;</span><span class="smarty-tag-text">Verify</span></a></div>');
		...
		this.showLoader = function(addr)
		{
			...
			//loaderUI.css("top", (lastFieldCorners.top + lastFieldCorners.height / 2 - loaderHeight / 2) + "px").css("left", (lastFieldCorners.right - loaderWidth - 10) + "px");
		...
		this.showAmbiguous = function(data)
		{
			...
			html += '</div></div></div>';
			$(html).hide().appendTo('#postcode').parent('.form-group').show(defaults.speed);
		...
		this.showInvalid = function(data)
		{
			...
			//top: '+corners.top+'px; left: '+corners.left+'px;
			var html = '<div class="smarty-ui" style=" width: '+corners.width+'px; height: '+corners.height+'px;">'
				+ '<div class="smarty-popup smarty-addr-'+addr.id()+'" style="width: '+(corners.width - 6)+'px; height: '+(corners.height - 3)+'px;">'
